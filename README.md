...
| `light` | number | ❌ | 补光强度，默认 `0`（不补光）；`>0` 时在相机就位后追加一盏**与相机视线同向的平行光**（无论相机指向任何方向，物体正面都被照亮），相机完成即销毁 |
...
> 请确保截图时场景具备合适照明。也可直接用 `--light <强度>` 让命令临时追加一盏**与相机视线同向**
> 的平行光补光（光线方向与相机方向完全一致，相机转向任何角度物体正面都亮），`light=0`（默认）则
> 不补光；该补光在相机渲染完成后立即销毁，不会留在场景里。
