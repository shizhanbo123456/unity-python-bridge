...
| 构建类 | 新建空物体、加组件、按名写属性/字段、反射创建 ScriptableObject 资产 | `gameobject.create` `component.add` `property.set` `asset.create` |
